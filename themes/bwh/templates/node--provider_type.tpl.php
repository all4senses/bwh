...
<?php endif; ?> <!-- if (!$page) -->
      ...
      <?php
        $url = 'http://bestwebhosting.ca' . ($_GET['q'] == 'home' ? '' : $_SERVER['REQUEST_URI']);
        
        $share_title = NULL;
                
        if ($is_front) {
          $share_title = bwh_misc_metatag_getFrontTitle();
        }
        ...
        echo '<div class="float share">' . bwh_blocks_getSocialiteButtons($url, $share_title) . '</div>';
      ?>
...
//          $results = $view->preview($display_name);
//          if ($view->result) {
//            module_load_include('inc', 'bwh_misc', 'inc/constants');
